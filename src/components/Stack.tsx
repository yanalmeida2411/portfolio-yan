...
export default function Stack() {
  ...
  return (
    <section
      id="stack"
      className="bg-[var(--color-field)] text-[var(--color-bg)]"
    >
      <div className="container py-[var(--section-y)]">
        <div className="section-head">
          <span className="num !text-[var(--color-accent-400)]">02</span>
          <h2 className="!text-[var(--color-bg)]">{t.stackTitle}</h2>
          <span className="rule !bg-[color-mix(in_srgb,var(--color-bg)_22%,transparent)]" />
          <span className="meta !text-[color-mix(in_srgb,var(--color-bg)_55%,transparent)]">
            {t.stackNote}
          </span>
        </div>
          ...
          {t.stack.map((g) => (
            ...
            >
              <div className="mb-[18px] flex items-baseline justify-between">
                ...
                <span className="text-[10px] text-[color-mix(in_srgb,var(--color-bg)_45%,transparent)]">
                  {g.ref}
                </span>
              </div>
              <ul className="m-0 flex list-none flex-col p-0">
                {g.items.map((item) => (
                  <li
                    key={item}
                    className="border-b border-[color-mix(in_srgb,var(--color-bg)_12%,transparent)] py-2 text-[14px]"
                  >
                    {item}
                  </li>
                ))}
